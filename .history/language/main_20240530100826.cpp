...
int maxAreaHelper(vector<int>& xs, int left, int right, int step)
{
    ...
    int volume_curr_tmp = (right_tmp - left_tmp) * std::min(xs[left_tmp], xs[right_tmp]);
    return std::max(volume_curr_tmp, std::max(volume_curr, std::max(volume_right_left, std::max(volume_left_right, std::max(volume_left, std::max(volume_right, maxAreaHelper(xs, left_tmp, right_tmp, step + 1))))));
}
