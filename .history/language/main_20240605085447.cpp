...
int uniquePathsWithObstaclesSolve(int m, int n, vector<vector<int>>& memo)
{
     if(m == 1 && n == 1)
       return 1;
     if(m == 0 && n != 0)
     ...
     if(memo[m][n] != -1)
       return memo[m][n];
     int paths = n == 1 ? 0 : uniquePathsWithObstaclesSolve(m - 1, n, memo) + n - 1 == 1 ? 0 : uniquePathsWithObstaclesSolve(m, n - 1, memo);
     memo[m][n] = paths;
     return paths;
}
...
int uniquePathsWithObstacles(vector<vector<int>>& obstacleGrid) 
{
     ...
     vector<vector<int>> memo = vector<vector<int>>(m + 1, vector<int>(n + 1, -1));
    return uniquePathsWithObstaclesSolve(m, n, memo);
} 
