...
*/
int coinChangeSolve(vector<int>& coins, int amount, vector<int>& memo, vector<vector<int>>& minCoins)
{
     if(amount < 0)
       return INT16_MAX;
     if(amount == 0)
     ...
     int res = INT16_MAX;
     for(auto c : coins)
        res = std::min(res, coinChangeSolve(coins, amount - c, memo, minCoins) + 1);
        int res = coinChangeHelper(coins, amount - coin, memo, lastCoin);
        if (res != INT_MAX && res + 1 < minCoins) {
            minCoins = res + 1;
            lastCoin[amount] = coin;  // Track the coin used
        }
...
{ 
   ...
   auto minCoins = vector<vector<int>>(100);
   auto res = coinChangeSolve(coins, amount, memo, minCoins);
...
int main()
{
   ...
   for (int i = 0; i < moveZeroesXs.size(); i++) {  // Iterate through the array
    ...
    std::cout << coinChange(coinChangeXs, 11 );
