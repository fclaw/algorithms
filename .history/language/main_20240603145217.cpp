...
vector<bool> makePossibility(vector<int> nums, int maxTarget)
{
     vector<bool> possible = vector<bool>(maxTarget);
     possible[0] = true;
     for (int k = 1; k <= nums.size(); k++) {
       for (int x = maxTarget - nums[k]; x >= 0; x--)
         possible[x + nums[k]] = possible[x + nums[k]] | possible[x];
     }
