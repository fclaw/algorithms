...
*/
void combinationSumHelper(size_t idx, vector<int> xs, int& target, vector<int>& ys, set<vector<int>>& acc)
{
    auto n = xs.size();
    if(target == 0)
      acc.insert(ys);
    ...
    else
    {
        for (size_t i = idx; i < n; i++)
        {
            ...
            ys.push_back(xs[i]);
            target -= xs[i];
            combinationSumHelper(i, xs, target, ys, acc);
            // Exclude the current element and move to the next index.
            ys.pop_back();
            target += xs[i];
            combinationSumHelper(i + 1, xs, target, ys, acc);
        }
    }
}
...
vector<vector<int>> combinationSum(vector<int>& candidates, int target)
{
    ...
    return std::vector(combinationXs.begin(), combinationXs.end());;
}
