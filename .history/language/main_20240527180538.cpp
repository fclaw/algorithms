...
vector<int> insertInterval(vector<vector<int>>& xs, vector<int>& x) 
{
    // find the position where new interval start time is between two intervals
    auto iter = std::lower_bound(xs.begin(), xs.end(), x, [](vector<int> x, vector<int> y) { return y[0] < x[0] && ; } );
...
int main()
{
   ...
   for (int i = 0; i < moveZeroesXs.size(); i++) {  // Iterate through the array
    ...
    for (int i = 0; i < insertIntervalsYs.size(); i++) {  // Iterate through the array
        std::cout << insertIntervalsYs[i] << " ";  // Print each element
    }
