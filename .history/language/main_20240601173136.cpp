...
int canCompleteCircuitSolve(vector<int>& gas, vector<int>& cost, int idx, int& step)
{
     return 0;
}
...
int canCompleteCircuit(vector<int>& gas, vector<int>& cost) 
{
    ...
    for(auto start : gas) {
      int init_step = 0;
      gas_station = std::max(gas_station, canCompleteCircuitSolve(gas, cost, start, init_step));
    }
    return gas_station;
}
