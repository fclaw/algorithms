...
Sort the array: Sorting helps to easily avoid duplicates and also allows us to use the two-pointer technique efficiently.
Iterate through the array: For each element in the array, treat it as a fixed element and try to find a pair of elements in the remaining part of the array which sums up to the negative of the fixed element.
Two-pointer technique: For the remaining elements after the fixed element, use two pointers to find pairs that sum up to the required value. Adjust the pointers based on whether the current sum is less than or greater than the required sum.
...
vector<vector<int>> threeSum(vector<int>& nums) 
{
    ...
    for (size_t i = 0; nums[i] <= 0; i++)
        for (size_t j = n - 1; nums[j] > 0; j--)
        {
            if(nums[i] + nums[j] <= 0)
              for (size_t k = j - 1; nums[k] >= 0; k--)
                if(nums[i] + nums[j] + nums[k] == 0)
                  res.insert({nums[i], nums[j], nums[k]});

            if(nums[i] + nums[j] >= 0)
              for (size_t l = i + 1; nums[l] < 0; l++)
                if(nums[i] + nums[j] + nums[l] == 0)
                  res.insert({nums[i], nums[j], nums[l]});
        }
