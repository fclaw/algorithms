...
void maxSubArray(vector<int>& nums, int i, int& best)
{
    if(i == 0)
      return nums[0];
    auto el = nums[i];
    auto curr_best = maxSubArray(nums, i - 1);
    return std::max(el, curr_best + el);
}
...
int main()
{
   ...
   for (int i = 0; i < moveZeroesXs.size(); i++) {  // Iterate through the array
    ...
    auto maxSum = maxSubArray(maxSubArrayXs, maxSubArrayXs.size() - 1);
    std::cout << maxSum;
}