...
int uniquePathsSolve(int m, int n, vector<vector<int>>& memo)
{
     ...
     int abovePaths = uniquePathsSolve(m - 1, n, memo);
     int leftPaths = uniquePathsSolve(m, n - 1, memo);
     memo[m - 1][n] = abovePaths;
     memo[m][n - 1] = leftPaths;
     return abovePaths + leftPaths;
}

int uniquePaths(int m, int n) 
{
    vector<vector<int>> memo = vector<vector<int>>(m + 1, vector<int>(n, -1));
