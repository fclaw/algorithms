...
int maxAreaHelper(vector<int>& xs, size_t left, size_t right, int vol)
{
    ...
    left_tmp = left_tmp == xs.size() - 1 ? left : left_tmp;

    size_t right_tmp = right;
    while(right_tmp-- > left_tmp && 
          xs[right] > xs[right_tmp]);
  
    right_tmp = right_tmp == 0 ? right : right_tmp;
    ...
    return std::max(volume_right_left, std::max(volume_left_right, std::max(vol, std::max(volume_left, std::max(volume_right, maxAreaHelper(xs, left_tmp, right_tmp, volume_curr))))));
}
...
int main()
{
   ...
   for (int i = 0; i < moveZeroesXs.size(); i++) {  // Iterate through the array
    ...
    vector<int> maxAreaXs = { 2, 3, 10, 5, 7,}; // expected: 36, got: 15
