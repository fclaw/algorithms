...
void recursive_insertion_sort(const int* array, size_t n) 
{
    if(n == 0)
    {
    ...
    {
        int* sorted_sub_array = recursive_insertion_sort(array, n - 1);
        int sorted_array[n];
        std::copy(array, array + n - 2, sorted_array);
        ...
        while(k > 0)
        {
            ...
            {
                std::swap(sorted_array[k - 1], sorted_array[k]);
                k--;
            }
            else 
            {
