...
ListNode* addTwoNumbersHelper(ListNode* first, ListNode* second, std::optional<int>& carryOverOne, ListNode* prev)
{   
     ...
     addTwoNumbersHelper(first->next, second->next, carryOverOne, node)->next = node;
}

ListNode* addTwoNumbers(ListNode* xs, ListNode* ys) 
{ 
  std::optional<int> carryOverOne = std::nullopt;
  ListNode* tail = nullptr;
  return addTwoNumbersHelper(xs, ys, carryOverOne, tail); 
}
...
{
      vector<vector<bool>> possibilities = vector<vector<bool>>(maxTarget + 1, vector<bool>(nums.size(), false));
      possibilities[0][0] = true;

      for (int k = 1; k <= nums.size(); k++) {
        for (int x = 0; x <= maxTarget; x++) { 
            if (x - nums[k] >= 0)
               possibilities[x][k] = possibilities[x][k - 1] | possibilities[x - nums[k]][k - 1];
...
int main()
{
   ...
   for (int i = 0; i < moveZeroesXs.size(); i++) {  // Iterate through the array
    ...
    auto possibilityYs = makePossibility(possibilityXs, 1);
