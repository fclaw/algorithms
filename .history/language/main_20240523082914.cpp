...
void combinationSumHelper(size_t idx, vector<int> xs, int target, vector<int>& ys, vector<vector<int>>& acc)
{
    ...
    else
    {
        ...
        ys.push_back(xs[idx]);
        target -= xs[idx];
        combinationSumHelper(idx, xs, target - xs[idx], ys, acc);
        // Exclude the current element and move to the next index.
        ys.pop_back();
        target += xs[idx];
        combinationSumHelper(idx + 1, xs, target + xs[idx], ys, acc);
    }
}
...
vector<vector<int>> combinationSum(vector<int>& candidates, int target)
{
    ...
    vector<int> ys = {};
    combinationSumHelper(0, candidates, 0, ys, combinationXs);
