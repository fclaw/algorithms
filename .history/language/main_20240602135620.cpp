...
int maxPathInGridSolve(vector<vector<int>> grid, int x, int y)
{
    if(x == 0 && y == 0)
      return 0;
    return std::max(maxPathInGridSolve(grid, x, y - 1), maxPathInGridSolve(grid, x - 1, y)) + grid[y][y];  
}

int maxPathInGrid(int m, int n)
{
    vector<vector<int>> grid = { {3, 7, 9, 2, 7}, {3, 7, 9, 2, 7}, {3, 7, 9, 2, 7}, {3, 7, 9, 2, 7}, {3, 7, 9, 2, 7} }       
}
