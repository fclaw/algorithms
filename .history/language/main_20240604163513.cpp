...
// solve(0, x) = 1
// solve(y, 0) = 1
int uniquePathsSolve(int m, int n)
{
     if(m == 0 && n == 0)
       return 0;
     if(m == 0 && n != 0)
       return uniquePathsSolve(0, n);
     if(m != 0 && n == 0)
       return uniquePathsSolve(m, n - 1);
     return uniquePathsSolve(m - 1, n) + uniquePathsSolve(m, n - 1);  
}

int uniquePaths(int m, int n) 
{
    return uniquePathsSolve(m, n);
}
