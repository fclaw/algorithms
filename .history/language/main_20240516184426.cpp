...
std::vector<std::vector<int>> generateAllSubset(std::vector<int> xs, std::vector<std::vector<int>>& xxs)
{
    if(xs.size() == 0)
      xxs.push_back({});
    else 
    {  
        for (size_t i = 0; i < xs.size(); i++)
        {
            auto tmp = xs;
            tmp.erase(tmp.begin());
            generateAllSubset(tmp, xxs);
            std::for_each(xxs.begin(), xxs.end(), [xs](std::vector<int>& ys) { ys.insert(ys.begin(), xs[0]); } );
        }
    }
}
