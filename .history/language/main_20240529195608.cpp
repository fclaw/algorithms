...
int maxAreaHelper(vector<int>& xs, size_t left, size_t right, int vol)
{
    if(left >= right)
      return vol;
    size_t left_tmp = left + 1;
    while(left < right && 
          xs[left] > xs[left_tmp])
      left_tmp++;

    size_t right_tmp = right - 1;
    while(right_tmp > left_tmp && 
          xs[right] > xs[right_tmp])
      right_tmp--;
  
    std::cout << left_tmp << " " << right_tmp << std::endl;

    if(left_tmp < right_tmp)
    {
      int volume_left = (right - left_tmp) * std::min(xs[left_tmp], xs[right]);
      int volume_right = (right_tmp - left) * std::min(xs[left], xs[right_tmp]);
      int volume_curr = (right_tmp - left_tmp) * std::min(xs[left_tmp], xs[right_tmp]);
      return std::max(vol, std::max(volume_left, std::max(volume_right, maxAreaHelper(xs, left_tmp, right_tmp, volume_curr))));
    }
    else return vol;
}
...
int maxArea(vector<int>& height)
{
     ...
     int volume = (right - left) * std::min(height[left], height[right]);
     return maxAreaHelper(height, left, right, volume);
}
