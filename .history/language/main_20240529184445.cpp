...
int maxAreaHelper(vector<int>& xs, size_t left, size_t right, int vol)
{
    ...
    int volume_curr = (right_tmp - left_tmp) * std::min(xs[left], xs[right_tmp]);
    return std::max(vol, std::max(volume_left, std::max(volume_right, maxAreaHelper(xs, left_tmp, right_tmp, )));
