...
void combinationSumHelper(size_t idx, size_t n, vector<int>& xs, int& sum, vector<int>& ys, int target, vector<vector<int>>& acc)
{
    ...
    else 
    {
        for (size_t i = idx; i < n; i++)
        {
            ...
            ys.push_back(xs[i]);
            sum += xs[i];
            combinationSumHelper(idx, n, xs, sum, ys, target, acc);
            // Exclude the current element and move to the next index.
            sum -= xs[i];
            ys.pop_back();
            xs.erase(xs.begin(), xs.begin() + i);
            combinationSumHelper(idx + 1, n, xs, sum, ys, target, acc);
        }
    }
}
...
int main()
{
   ...
   for (int i = 0; i < moveZeroesXs.size(); i++) {  // Iterate through the array
    ...
    std::vector<int> combinationSumXs = {2, 3};
    auto combinationSumYs = combinationSum(combinationSumXs, 7);
