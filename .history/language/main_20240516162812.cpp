...
std::vector<std::vector<int>> permuteHelper(std::vector<int>& xs, size_t n)
{
    if(n == 0)
      return {{xs[0]}};
    auto yys = permuteHelper(xs, n - 1);
    std::vector<std::vector<int>> tmp_xs = {};
    for (size_t i = 0; i < yys.size(); i++)
    {
        auto arr_s = yys[i];
        arr_s.insert(arr_s.begin(), xs[n]);
        tmp_xs.push_back(arr_s);

        for (size_t j = 0; j < yys[i].size(); j++)
        {
            auto zs = yys[i];
            zs[j] = xs[n];
            auto zss = permuteHelper(zs, n - 1);
            for (size_t k = 0; k < zss.size(); k++)
            {
                zss[k].insert(zss.begin(), zs[j]);
                tmp_xs.push_back(zss[k]);
            }
...
void generateAllSubset(std::vector<int> xs, std::vector<std::vector<int>>& xxs)
{
    if(xs.size() == 0)
      xxs.push_back({});
    ...
    {
        xs.erase(xs.begin());
        generateAllSubset(xs, xxs);
        std::for_each(xxs.begin(), xxs.end(), [xs, i](std::vector<int>& ys) { ys.insert(ys.begin(), xs[i]); } );
    }
...
int main()
{
    ...
    std::vector<int> permutation_xs = {1, 2 };
    auto permutation_xxs = permuteHelper(permutation_xs, 1);

    for ( const auto &row : permutation_xxs )
    {
