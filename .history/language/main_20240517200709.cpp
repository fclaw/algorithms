...
void permuteHelperV2(std::vector<int> xs, std::vector<std::vector<int>>& acc)
{
    if(xs.size() == 0)
       acc.push_back({});
    for (size_t i = 0; i < xs.size(); i++)
    {
      ...
      tmp.erase(tmp.begin() + i);
      auto el = *(tmp.begin() + i);
      auto yys = permuteHelperV2(tmp, acc);
      std::for_each(
        yys.begin(), 
        yys.end(),
        [xs, i, &acc](std::vector<int>& ys)
        { ys.insert(ys.begin(), xs[i]);
          acc.push_back(ys);
        } );
    }
...
int main()
{
    ...
    std::vector<int> permutationXs = {1, 2, 3, 4};
