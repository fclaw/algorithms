...
std::vector<std::vector<int>> threeSum(std::vector<int>& nums)
{
    ...
    for (size_t i = 0; nums[i] < 0; i++)
    {
        if(i > 0 && nums[i] == nums[i - 1])
          continue;
        size_t negative_ptr = i + 1;
        size_t positive_ptr = n - 1;
        while(negative_ptr < positive_ptr)
        {
            auto zero = nums[i] + nums[negative_ptr] + nums[positive_ptr];
            if(zero == 0)
            {
               res.push_back({nums[i], nums[negative_ptr], nums[positive_ptr]});
               break;
            }
            if(zero > 0)
              positive_ptr--;
            if(nums[i] + nums[negative_ptr] + nums[positive_ptr] < 0)
              negative_ptr++;
        }
    }
    return res;
