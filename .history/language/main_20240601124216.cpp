...
int coinAllSolutions(vector<int>& coins, int amount)
{
    if(amount < 0)
      return 0;
    if(amount == 0)
    ...
    int solutions = 0;
    for(auto c : coins)
      solutions += coinAllSolutions(coins, amount - c);
    return solutions;
}
...
int main()
{
   ...
   for (int i = 0; i < moveZeroesXs.size(); i++) {  // Iterate through the array
    ...
    auto coinAllSolutionsXs = {1, 3, 4};
    std::cout << coinAllSolutions(coinAllSolutionsXs, 5);
}