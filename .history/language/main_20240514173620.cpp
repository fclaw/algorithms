...
vector<string> binaryTreePathsHelper(TreeNode* root)
{
    ...
    if(root->left == nullptr && root->right == nullptr)
      return std::vector<string>(root->val);
    ...
    auto merged = std::vector<string>(left_s.size() + right_s.size());
    return {}; // std::merge(left_s.begin(), left_s.end(), right_s.begin(), right_s.end(), merged.begin());
}
