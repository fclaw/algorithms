...
std::vector<std::vector<int>> permuteHelper(std::vector<int> xs, std::vector<std::vector<int>>& xxs)
{
    if(xs.size() == 1)
       return {{xs[0]}};
    ...
    for (size_t i = 0; i < xs.size(); i++)
    {
      ...
      auto el = *(tmp.begin() + i);
      auto yys = permuteHelper(tmp, xxs);
      std::for_each(
        yys.begin(), 
        yys.end(),
...
int main()
{
    ...
    auto acc = permuteHelper(permutationXs, allPermutations);
