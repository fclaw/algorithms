...
std::vector<std::vector<int>> treePath(TreeNode* root)
{
   
    auto findPathSums = [&](auto&& findPathSums, TreeNode* node, int currentSum, std::vector<int>& pathSums) -> void {
    if (node == nullptr) {
    ...
    if (node->left == nullptr && node->right == nullptr) {
        ...
        findPathSums(node->left, currentSum, pathSums);
        findPathSums(node->right, currentSum, pathSums);
    }
  };

  auto root = nullptr;
   
  findPathSums(findPathSums, root, 0, {});
