...
std::optional<int> binarySearchJump(vector<int> xs, int target, bool isCalculated[], int resXs[])
{
     return std::make_optional(0);
}


int coinChangeSolve(vector<int>& coins, int amount)
{
     if(amount < 0)
       return INT16_MAX;
     if(amount == 0)
     ...
     int res = INT16_MAX;
     for(auto c : coins)
        res = std::min(res, coinChangeSolve(coins, amount - c) + 1);
     return res;
}
