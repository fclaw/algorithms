...
std::vector<std::vector<int>> permuteHelper(std::vector<int> xs, std::vector<std::vector<int>>& acc, std::vector<std::vector<int>>& xxs)
{
    if(xs.size() == 1)
       acc.push_back({xs[0]});
    for (size_t i = 0; i < xs.size(); i++)
    {
      ...
      permuteHelper(tmp, acc_x, xxs);
      std::for_each(
        acc_x.begin(), 
        acc_x.end(),
        [el](std::vector<int>& ys)
        { ys.insert(ys.begin(), el);
        } );  
    }
}
...
int main()
{
    ...
    std::vector<int> permutationXs = {1, 2};
    std::vector<std::vector<int>> allPermutations = {};
    std::vector<std::vector<int>> acc = {};
    permuteHelper(permutationXs, acc, allPermutations);

    for ( const auto &row : allPermutations )
    {
      for ( const auto &s : row ) 
        std::cout << s << ' ';
      std::cout << std::endl;
