...
vector<vector<int>> permuteHelper(vector<int>& xs, size_t n)
{
    if(n == 0)
      return {{xs[0]}};
    auto yys = permuteHelper(xs, n - 1);
    vector<vector<int>> tmp_xs = {};
    for (size_t i = 0; i < yys.size(); i++)
    {
        auto arr_s = yys[i];
        auto arr_e = yys[i];
        arr_e.push_back(xs[n]);
        arr_s.insert(arr_s.begin(), xs[n]);
        tmp_xs.push_back(arr_s);
        tmp_xs.push_back(arr_e);
    }
    return tmp_xs;
}
...
int main()
{
    ...
    auto permutation_xs = permuteHelper(permutation_xs, 2);

        for (int i = 0; i < permutation_xs.size(); i++) {  // Iterate through the array
        std::cout << array_1[i] << " ";  // Print each element
    }
