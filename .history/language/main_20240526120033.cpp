...
vector<int> sortColors(vector<int>& nums) 
{
    ...
    // for (size_t i = 1; i < tmp[0]; i++)
    //   nums[idx++] = tmp[0];
    // for (size_t i = 1; i < tmp[1]; i++)
    //   nums[idx++] = tmp[1];
    // for (size_t i = 1; i < tmp[2]; i++)
    //   nums[idx++] = tmp[2];
}
...
int main()
{
   ...
   for (int i = 0; i < moveZeroesXs.size(); i++) {  // Iterate through the array
    ...
    vector<int> sortColorsXs = {2, 0, 2, 1, 1, 0};
    auto ys = sortColors(sortColorsXs);

    for (int i = 0; i < ys.size(); i++) {  // Iterate through the array
        std::cout << ys[i] << " ";  // Print each element
    }
