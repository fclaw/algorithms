...
void combinationSumHelper(size_t idx, size_t n, vector<int>& xs, int sum, vector<int>& ys, int target, vector<vector<int>>& acc)
{
    if(sum == target)
      ...
      return;
        ...
        {
            ys.push_back(xs[i]);
            combinationSumHelper(idx, n, xs, sum + xs[i], ys, target, acc); // Include the current element in the combination and recursively call the function with the updated combination and target.
