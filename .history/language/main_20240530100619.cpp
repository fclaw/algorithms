...
int maxAreaHelper(vector<int>& xs, int left, int right, int step)
{
    if(left >= right)
      return 0;
    int left_tmp = left;
    ...
    std::cout << step << ": --> " << left << ", " << left_tmp << ", " << right << ", " << right_tmp << std::endl;
    ...
    int volume_curr = (right - left) * std::min(xs[left], xs[right]);
    return std::max(volume_curr, std::max(volume_right_left, std::max(volume_left_right, std::max(volume_left, std::max(volume_right, maxAreaHelper(xs, left_tmp, right_tmp, step + 1))))));
}

int maxArea(vector<int>& height)
{
     return maxAreaHelper(height, 0, height.size() - 1, 0);
}
   ...
   for (int i = 0; i < moveZeroesXs.size(); i++) {  // Iterate through the array
    ...
    vector<int> maxAreaXs = 
       { 76,155,15,188,180,154,84,34,187,142,22,5,27,183,111,128,50,58,2,
         112,179,2,100,111,115,76,134,120,118,103,31,146,58,198,134,38,104,170,25,92,112,199,49,140,135,160,20,185,171,23,98,150,177,198,61,92,26,147,164,144,51,196,42,109,194,177,100,99,99,125,143,12,76,192,152,11,152,124,197,123,147,95,73,124,45,86,168,24,34,133,120,85,81,163,146,75,92,198,126,191 };
