...
int canCompleteCircuitSolve(vector<int>& gas, vector<int>& cost, int idx, int& tank, int& step)
{
    ...
    auto tmp = step;
    step += 1;
    auto cost_idx = idx + tmp >= gas.size() ? (idx + tmp) % gas.size() : idx + tmp;
    auto gas_idx = idx + step >= gas.size() ? (idx + step) % gas.size() : idx + step;
    tank -= cost[cost_idx];
...
int canCompleteCircuit(vector<int>& gas, vector<int>& cost) 
{
    ...
    return gas_station == INT32_MIN ? -1 : gas_station + 1;
}
