...
int maxAreaHelper(vector<int>& xs, size_t left, size_t right, int& step)
{
    ...
    return std::max(volume_curr, std::max(volume_right_left, std::max(volume_left_right, std::max(volume_left, std::max(volume_right, maxAreaHelper(xs, left_tmp, right_tmp, s))))));
}
