...
int canCompleteCircuitSolve(vector<int>& gas, vector<int>& cost, int idx, i int& step)
{
     return 0;
}
...
int canCompleteCircuit(vector<int>& gas, vector<int>& cost) 
{
    ...
    for(auto start : gas) {
      int step = 0;
      int tank = 0;
      gas_station = std::max(gas_station, canCompleteCircuitSolve(gas, cost, start, tank, step));
    }
    return gas_station;
}
