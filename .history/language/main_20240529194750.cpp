...
int maxAreaHelper(vector<int>& xs, size_t left, size_t right, int vol)
{
    ...
    return std::max(vol, std::max(volume_left, std::max(volume_right, maxAreaHelper(xs, left_tmp + 1, right_tmp - 1, volume_curr))));
}
...
int maxArea(vector<int>& height)
{
     ...
     return maxAreaHelper(height, left + 1, right - 1, volume);
}
