...
*/
void combinationSumHelper(size_t idx, size_t n, vector<int>& xs, int& target, vector<int>& ys, std::set<vector<int>>& acc)
{
    if(target == 0)
      acc.insert(ys);
    else if(target < 0)
    ...
    else
    {
        for (size_t i = idx; i < n; i++)
        {
            ...
            ys.push_back(xs[i]);
            target -= xs[i];
            combinationSumHelper(i, n, xs, target, ys, acc);
            // Exclude the current element and move to the next index.
            target += xs[i];
            ys.pop_back();
            combinationSumHelper(i + 1, n, xs, target, ys, acc);
        }
    }
}
...
{
    set<vector<int>> combinationXs = {};
    for (size_t i = 0; i < candidates.size(); i++)
    {
      vector<int> ys = {candidates[i]};
      vector<int> xs = candidates;
      xs.erase(xs.begin(), xs.begin() + (i == 0 ? 0 : i - 1));
      int val = target - candidates[i];
      combinationSumHelper(i, xs.size(), xs, val, ys, combinationXs);
    }

    vector<vector<int>> tmpXs = std::vector(combinationXs.begin(), combinationXs.end());
    return std::copy(combinationXs.begin(), combinationXs.end(), std::back_inserter(tmpXs));;
}
...
int main()
{
   ...
   for (int i = 0; i < moveZeroesXs.size(); i++) {  // Iterate through the array
    ...
    auto combinationSumYs = combinationSum(combinationSumXs, 8);
