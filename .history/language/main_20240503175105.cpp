...
std::tuple<size_t, size_t> sumOfTwoNumbers(const std::vector<int>& array, int target)
{
...
int main()
{
    ...
    std::cout << binary_search(array_2, 0, array_2.size(), 1);

    std::vector<int> twoSum_array = {2, 7, 11, 15};
    auto tpl = sumOfTwoNumbers(twoSum_array, 9);
    std::cout << std::get tpl
}