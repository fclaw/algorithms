...
{
      vector<vector<bool>> possibilities = vector<vector<bool>>(nums.size() + 1, vector<bool>(maxTarget + 1, false));
      possibilities[0][0] = true;

      for (int k = 1; k < nums.size(); k++) {
        for (int x = 0; x <= maxTarget; x++) { 
            if (x - nums[k] >= 0)
               possibilities[k][x] = possibilities[k][x] | possibilities[k - 1][x - nums[k]];
            possibilities[k][x] = possibilities[k][x] | possibilities[k - 1][x];
        }
      }
