...
int coinChangeSolve(vector<int>& coins, int amount, bool isCalculated[], int resXs[])
{
     if(amount < 0)
       return INT16_MAX;
     if(amount == 0)
       return 0;
     if(isCalculated[amount])
       return resXs[amount];

     int res = INT16_MAX;
     for(auto c : coins) {
        auto cRes = coinChangeSolve(coins, amount - c, isCalculated, resXs);
        isCalculated[amount - c] = true;
        resXs[amount - c] = cRes; 
        res = std::min(res, coinChangeSolve(coins, amount - c, isCalculated, resXs) + 1);   
     return res;
}
...
{ 
   bool isCalculated[100000];
   int resXs[100000];
   auto res = coinChangeSolve(coins, amount, isCalculated, resXs);
...
int main()
{
   ...
   for (int i = 0; i < moveZeroesXs.size(); i++) {  // Iterate through the array
    ...
    vector<int> coinChangeXs = { 2 };
    std::cout << coinChange(coinChangeXs, 3);
