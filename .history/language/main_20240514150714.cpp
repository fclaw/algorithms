...
std::tuple<TreeNode*, bool> removeLeafNodesHelper(TreeNode* root, int target)
{
    if(root == nullptr)
    ...
    if(std::get<0>(left_node) == nullptr &&
       std::get<0>(right_node) == nullptr &&
       root->val == target)
      return std::make_tuple(nullptr, true);
    else 
    {
        auto left = std::get<1>(left_node) ? nullptr : root->left;
        auto right = std::get<1>(right_node) ? nullptr : root->right;
        root->left = left;
        root->right = right;
        return std::make_tuple(left == nullptr && right == nullptr &&root->val == target ? nullptr : root, left == nullptr && right == nullptr);
    }
}
...
{
    return std::get<0>(removeLeafNodesHelper(root, target));
}
...
int main()
{
    ...
    auto tree = removeLeafNodes(test_tree, 2);
