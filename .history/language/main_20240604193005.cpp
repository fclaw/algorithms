...
int robHelper(vector<int>& xs, int i)
{
    if(i > xs.size())
      return 0;
    int maxBooty = INT32_MIN;
    for (size_t i = i + 2; i < xs.size(); i++)
      maxBooty = std::max(maxBooty, robHelper(xs, i));
    return maxBooty;
}
...
int rob(vector<int>& nums) 
{
    int maxBooty = INT32_MIN;
    for (size_t i = 0; i < nums.size(); i++)
      maxBooty = std::max(maxBooty, robHelper(nums, i));
    return maxBooty;
}
