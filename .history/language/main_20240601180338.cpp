...
int canCompleteCircuitSolve(vector<int>& gas, vector<int>& cost, int idx, int& tank, int& step)
{
    ...
    step += 1;
    auto curr_idx = idx + step >= gas.size() ? (idx + step) % gas.size() : idx + step;
    tank = tank - cost[curr_idx - 1 < 0 ? 0 : curr_idx - 1] + gas[curr_idx];

    std::cout << "idx: " << idx << ", curr_idx: " << (idx + step) % gas.size() << ", tank" << tank << std::endl;
...
{
    int gas_station = INT32_MIN;
    for(int i = 0; i < gas.size(); i++) {
      int step = 0;
      int tank = gas[i];
      gas_station = std::max(gas_station, canCompleteCircuitSolve(gas, cost, i, tank, step));
    }
    return gas_station; //  == INT32_MIN ? -1 : gas_station;
}
