...
// r(1) = P(0) * S(2) -> 1 * 60 -> 60
// r(2) = P(1) * S(3) -> 2 * 20 -> 40
std::vector<int> computeSuffixProduct(const std::vector<int>& xs) {
    int n = xs.size();
    std::vector<int> s(n);
    s[n - 1] = xs[n - 1];
    for (int i = n - 2; i >= 0; --i) {
...
std::vector<int> computePrefixProduct(const std::vector<int>& xs) {
    int n = xs.size();
    std::vector<int> p(n);
    p[0] = xs[0];
    for (int i = 1; i < n; ++i) {
...
int main()
{
   ...
   for (int i = 0; i < moveZeroesXs.size(); i++) {  // Iterate through the array
    ...
    vector<int> productExceptSelfXs = {1, 2, 3, 4};
    auto productExceptSelf(productExceptSelfXs);
