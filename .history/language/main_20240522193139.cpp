...
*/
void combinationSumHelper(size_t idx, size_t n, vector<int>& xs, int& sum, vector<int>& ys, int target, vector<vector<int>>& acc)
{
    if(sum == target)
      acc.push_back(ys);
    else if(sum > target)
    ...
    else 
    {
        for (size_t i = idx; i < n; i++)
        {
            ...
            ys.push_back(xs[i]);
            sum += xs[i];
            combinationSumHelper(i, n, xs, sum, ys, target, acc);
            // Exclude the current element and move to the next index.
            sum -= xs[i];
            ys.pop_back();
            combinationSumHelper(i + 1, n, xs, sum, ys, target, acc);
        }
    }
}

void combinationSum(vector<int>& candidates, int target, vector<vector<int>>& res)
{
    for (size_t i = 0; i < candidates.size(); i++)
    {
      vector<int> ys = {candidates[i]};
      vector<int> xs = candidates;
      xs.erase(candidates.begin(), candidates.begin() + i) ;
      combinationSumHelper(i, candidates.size(), xs, candidates[i], ys, target, res);
    }
}
...
int main()
{
   ...
   for (int i = 0; i < moveZeroesXs.size(); i++) {  // Iterate through the array
    ...
    std::vector<int> combinationSumXs = {1, 2};
    std::vector<std::vector<int>> combinationSumRes = {};
    auto combinationSumYs = combinationSum(combinationSumXs, 8, combinationSumRes);
   
    for ( const auto &row : combinationSumRes )
    {
      for ( const auto &s : row ) std::cout << s << ' ';
      std::cout << std::endl;
    }
