...
int maxPathInGridSolve(vector<vector<int>> grid, int x, int y)
{    
    if(x == 3 || y == 3)
      return 0;
    auto fromDown = maxPathInGridSolve(grid, y - 1, x);
    auto fromLeft = maxPathInGridSolve(grid, y, x - 1);
    return std::max(fromDown, fromLeft) + grid[y][x];
    // maxPathInGridSolve(grid, y, x - 1)) + grid[y][x];  
}

int maxPathInGrid(vector<vector<int>> grid) { return maxPathInGridSolve(grid, 0, 0); }
