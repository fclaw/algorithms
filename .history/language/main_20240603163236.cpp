...
/*
To check if a linked list has a cycle, you can use Floyd's Tortoise and Hare algorithm, which is a two-pointer technique. This approach involves using two pointers that traverse the list at different speeds. If there is a cycle, the faster pointer will eventually meet the slower pointer
*/
bool hasCycle(ListNode *head) 
{
     ...
     {
         if(nodes.find(ptr->next->val) != nodes.end())
         {
              isCycle = true;
              break;
         }
         nodes.insert({ptr->val, ptr->next->val});
         ptr = ptr->next;
     }
