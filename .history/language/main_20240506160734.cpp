...
std::optional<std::tuple<int, int>> maxProfit(vector<int>& prices) 
{
    int max_profit = 0;
    int buy_day;
    int sell_day;
    ...
    {
        for (size_t j = i + 1; i < prices.size(); j++)
        {
...
int main()
{
    ...
    auto profits_days = maxProfit(prices)
    std::cout << (profits_days.has_value() ? ()"buy: " ++ std::to_string(std::get<1>(profits_days.value())) ++ ", sell: " ++ std::to_string(std::get<2>(profits_days.value()))  : "no profit");
}