...
// solve(0, x) = 1
// solve(y, 0) = 1
int uniquePathsSolve(int m, int n, vector<int>& memo)
{
     if(m == 1 && n == 1)
       return 1;
     if(m == 0 && n != 0)
     ...
     if(memo[m * n] != -1)
       return memo[m * n];
     auto paths = uniquePathsSolve(m - 1, n, memo) + uniquePathsSolve(m, n - 1, memo);
     memo[m * n] = paths;
     return paths;
}
...
{
    vector<vector<int>> memo = vector<vector<int>>(m, vector<int>(m, -1));
