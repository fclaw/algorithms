...
{
    int forward_ptr = 0;
    int backward_ptr = nums.size();
    while(nums.size() > 0 && forward_ptr < backward_ptr)
    {
        if(nums[forward_ptr] == val)
        {
            while(nums[--backward_ptr] == val && backward_ptr > forward_ptr)
...
int main()
{
    ...
    std::vector<int> removeElement_array = {0,1,2,2,3,0,4,2};
    std::cout << removeElement(removeElement_array, 2);
