...
// solve(0, x) = 1
// solve(y, 0) = 1
int uniquePathsSolve(int m, int n, unordered_map<std::pair<int, int>, int>& memo)
{
     if(m == 1 && n == 1)
       return 1;
     if(m == 0 && n != 0)
     ...
     if(m != 0 && n == 0)
       return 0;
     if(auto el = memo.find(std::make_pair(m, n)); el != memo.end())
     { 
       return el->second;
     }
     int paths = uniquePathsSolve(m - 1, n, memo) + uniquePathsSolve(m, n - 1, memo);
     memo[std::make_pair(m, n)] = paths;
     return paths;
}

int uniquePaths(int m, int n) 
{
    unordered_map<std::pair<int, int>, int> memo = unordered_map<std::pair<int, int>, int>();
