...
ListNode* addTwoNumbersHelper(ListNode* first, ListNode* second, ListNode* curr)
{   
    ...
    //  node->val = x > 10 ? x % 10 : x + carryOverOne.value();
    //  carryOverOne = x > 10 ? std::make_optional(1) : std::nullopt;

    //  addTwoNumbersHelper(first->next, second->next, node);
    return nullptr;
}
...
ListNode* addTwoNumbers(ListNode* xs, ListNode* ys) 
{
  ...
  return addTwoNumbersHelper(xs, ys, tail); 
}
...
vector<bool> makePossibility(vector<int> nums, int m)
{
      vector<vector<bool>> possibilities = vector<vector<bool>>(m + 1, vector<bool>(nums.size(), ));
      possibilities[0] = true;
      for (int k = 0; k < nums.size(); k++)
        for (int x = m-nums[k]; x >= 0; x--)
          possibilities[x+nums[k]] = possibilities[x+nums[k]] | possibilities[x];
...
int main()
{
   ...
   for (int i = 0; i < moveZeroesXs.size(); i++) {  // Iterate through the array
    ...
    vector<int> possibilityXs = {1, 3, 3, 5};
    auto possibilityYs = makePossibility(possibilityXs, 10);

    printVector(possibilityYs);
