...
{
    std::sort(intervals.begin(), intervals.end(), [](vector<int> x, vector<int> y) { return if x[0] < y[0] ; } );
    auto interim_interval = intervals[0];
    vector<vector<int>> res = {};
    for (size_t i = 1; i < intervals.size(); i++)
    {
        if(intervals[i][0] >= interim_interval[0] && 
           intervals[i][0] < interim_interval[1] &&
           intervals[i][1] <= interim_interval[1] ||
           intervals[i][1] == interim_interval[1] && 
           intervals[i][0] == intervals[i][1]
           )
          continue;
        else if(intervals[i][0] == interim_interval[0] &&
                intervals[i][1] > interim_interval[1])
           interim_interval[1] = intervals[i][1];
        else if(intervals[i][0] > interim_interval[0] && 
                intervals[i][0] <= interim_interval[1] &&
                intervals[i][1] > interim_interval[1])
            interim_interval[1] = intervals[i][1];
