...
*/
void combinationSumHelper(size_t idx, size_t n, vector<int> xs, int target, vector<int>& ys, vector<vector<int>>& acc)
{
    if(target == 0)
      acc.push_back(ys);
    else if(target < 0)
    ...
    else
    {
        for (size_t i = idx; i < n; i++)
        {
            ...
            ys.push_back(xs[i]);
            auto targetM = target - xs[i];
            combinationSumHelper(i, n, xs, targetM, ys, acc);
            // Exclude the current element and move to the next index.
            auto targetP = target + xs[i];
            ys.pop_back();
