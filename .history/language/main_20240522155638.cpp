...
1 Sort the Array: Sorting the array can help in making the algorithm more efficient by allowing early termination of recursive calls.

2 Backtracking Function: Define a recursive function that will try to build combinations of numbers. This function will:

Add the current number to the current combination.
Recursively call itself with the updated sum and combination.
Backtrack by removing the last added number and trying the next one.
Base Case and Recursive Case:

3 Base Case: When the current sum equals the target, add the current combination to the result.
Recursive Case: For each number in the array, add it to the current combination and call the function recursively with the remaining sum (target minus current number).
Avoid Duplicates: Ensure the solution does not include duplicate combinations by always moving forward in the array and avoiding the reuse of the same index.
*/
