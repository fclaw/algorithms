...
int uniquePathsWithObstaclesSolve(int m, int n, vector<vector<int>>& memo) { return 1; }

int uniquePathsWithObstacles(vector<vector<int>>& obstacleGrid)
{
     vector<vector<int>> dp = vector<vector<int>>(obstacleGrid.size(), vector<int>(obstacleGrid[0].size(), 0));

    // base case 0, 0
    dp[0][0] = 1;

    // x is fixed, y is zero 
} 
