...
void permuteHelper(std::vector<int> xs, std::vector<std::vector<int>>& acc, std::vector<std::vector<int>>& xxs)
{
    if(xs.size() == 1)
       acc.push_back({xs[0]});
    for (size_t i = 0; i < xs.size(); i++)
    {
      ...
      std::for_each(
        acc_x.begin(), 
        acc_x.end(),
        [tmp, i, &xxs](std::vector<int>& ys)
        { ys.insert(ys.begin(), tmp[i]);
          xxs.push_back(ys);
        } );
    }
}
...
int main()
{
    ...
    std::vector<int> permutationXs = {1, 2, 3};
