...
{
    int nums1_ptr = 0;
    int nums2_ptr = 0; // the second array pointer
    int nums1_after_m_loop1_ptr = m;
    while(nums1_ptr < m)
    {
        ...
        {
            nums1[nums1_after_m_loop1_ptr] = nums1[nums1_ptr];
            std::swap(nums1[nums1_ptr], nums2[nums2_ptr]);
            nums2_ptr++;
            nums1_ptr++;
            nums1_after_m_loop1_ptr++;
        }
    }

    int nums1_after_m_loop2_ptr = m;
    while(nums2_ptr < n)
    {
        if(nums1[nums1_after_m_loop2_ptr] == 0)
        {
            nums1[nums1_after_m_loop2_ptr] = nums2[nums2_ptr];
            nums1_after_m_loop2_ptr++;
            nums2_ptr++;   
        }
        else if(nums1[nums1_after_m_loop2_ptr] < nums2[nums2_ptr])
          nums1_after_m_loop2_ptr++;
        else
        {
            nums1[nums1_after_m_loop2_ptr + 1] = nums1[nums1_after_m_loop2_ptr];
            std::swap(nums1[nums1_after_m_loop2_ptr], nums2[nums2_ptr]);
            nums1_after_m_loop2_ptr++;
            nums2_ptr++;
        }
    }
