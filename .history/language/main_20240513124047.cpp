...
std::tuple<int, int> averageOfSubtree(TreeNode* root, int& x, std::vector<std::tuple<int, int>>& xs)
{
    if(root == nullptr)
      return std::make_tuple(0, 1);

    auto left = averageOfSubtree(root->left, x, xs);
    auto right = averageOfSubtree(root->right, x, xs);
    auto val = root->val + std::get<1>(left) + std::get<1>(right);
    auto cnt = 1 + std::get<2>(left) + std::get<2>(right);
    xs.push_back(std::make_tuple(val, cnt));
    if ()
